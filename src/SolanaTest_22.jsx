...
import {TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_2022_PROGRAM_ID} from "@solana/spl-token/src/constants.js";
...
function SolanaTest() {
    ...
    const initLibreplex_token2022 = async () => {
        setTxHash('');
        ...
        const deployment_config = PublicKey.findProgramAddressSync([Buffer.from("deployment_config"), deployment.toBuffer()], programPublicKey)[0];
        const initParam = {
            ticker: ticker,
            ...
            deploymentType: 0,
            requireCreatorCosign: false,
            creatorFeeTreasury: new PublicKey(account),
            creatorFeePerMintInLamports: new BN(LAMPORTS_PER_SOL * 0),
            useInscriptions: true
        }
        const txHash = await program.methods.initialiseV2(initParam).accounts({
    ...
    const deployLibreplex_token2022 = async () => {
        setTxHash('');
        const ticker = `arf`;
        const provider = new AnchorProvider(connection, window.solana, AnchorProvider.defaultOptions());
        ...
        const program = new Program(replex_idl, programPublicKey, provider);
        const run = program.methods.deployToken22().accounts({
            systemProgram: web3.SystemProgram.programId,
            payer: new PublicKey(account),
            deployment: deployment,
            ...
            fungibleMint: fungibleMintKey.publicKey,
            fungibleEscrowTokenAccount: getAssociatedTokenAddressSync(fungibleMintKey.publicKey, deployment, true, TOKEN_2022_PROGRAM_ID),
            hashlist: PublicKey.findProgramAddressSync([Buffer.from("hashlist"), deployment.toBuffer()], programPublicKey)[0]
        });
    ...
    const initLibreplex = async () => {
        ...
        const txHash = await program.methods.initialise(initParam).accounts({
            systemProgram: web3.SystemProgram.programId,
            payer: new PublicKey(account),
            deployment: PublicKey.findProgramAddressSync([Buffer.from("deployment"), Buffer.from(ticker)], programPublicKey)[0]
        }).rpc({
            skipPreflight: true,
        });
        console.log(`Tx Complete: https://explorer.solana.com/tx/${txHash}?cluster=devnet`)
